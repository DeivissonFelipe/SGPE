@extends('layouts.adminlte')

@push('tituloAba')
   Planos
@endpush


@push('css')
    <link href="{{ asset('/css/ajax.css')}}" rel="stylesheet" type="text/css" />
    <link href="{{ asset('/css/plano.css')}}" rel="stylesheet" type="text/css" />
    <style>
    .modal_zindex {
        z-index: 1151 !important;
    }
</style>
@endpush


@section('breadcrumb')

<h1>
    Planos
    <small>View</small>
</h1>

{!! Breadcrumbs::render('planoId', $plano) !!}

@endsection


@push('javascript')
<script>
    $(document).ajaxStart(function(){ 
        $('#envio_aprovacao').html('<i class="fa fa-hourglass-o" aria-hidden="true"></i> Carregando...'); 
    });

    $(document).ajaxStop(function(){ 
        $('#envio_aprovacao').html('<i class="fa fa-paper-plane"></i> Enviar'); 
    });

    $(document).ready(function(){
        $("[data-tt=tooltip]").tooltip();
        $('#envio_aprovacao').click(function(event){
            event.preventDefault();
            $.ajax({
                type: 'GET',
                url: '/planos/{{$plano->id}}/warnings',
                dataType: 'json',
                success: function (data) {
                    if(!$.trim(data)){
                        window.location = '/planos/'+$('#envio_aprovacao').attr('data-idPlano')+'/verificacao';
                    }else{
                        var msg = '';
                        $.each(data, function(index, value){
                            msg = "<li>";
                            msg += value;
                            msg += "</li>";
                        });
                        $('#msg-modal').html(msg);
                        $('#confirm_envio').modal('show');
                    }
                },
                error: function (data) {
                    console.log('Error:', data);
                }
            });
        });
    });
</script>
            
@endpush



@section('content')
<div class="row">
              
    <!-- MENU -->
    <div class="col-md-8 col-md-offset-2">
        <div class="btn-group btn-group-justified" role="group" arial-label="menu" style="margin-bottom:1px">
            @can('owner', $plano)
            <div class="btn-group" style="border:1px solid white">
                <a class="btn btn-app btn-ufop" href="/planos/{{$plano->id}}/edit" data-toggle="tooltip" title="Edite as informações do plano"><i class="fa fa-edit"></i> Editar</a>
            </div>
            @if($plano->status =="Em Edição")
                <div class="btn-group" style="border:1px solid white">
                    <a class="btn btn-app btn-ufop"  id="envio_aprovacao"  data-tt="tooltip" data-idPlano="{{$plano->id}}" title="Enviar este plano de ensino para aprovação da assembléia."><i class="fa fa-paper-plane"></i> Enviar</a>    
                </div>
            @endif
            @endcan
            <div class="btn-group" style="border:1px solid white">
                <a class="btn btn-app btn-ufop" href="/planos/{{$plano->id}}/export" data-toggle="tooltip" title="Exporte os dados deste plano de ensino para um plano seu."><i class="fa fa-external-link"></i> Exportar</a>
            </div>
            <div class="btn-group" style="border:1px solid white">
                <a class="btn btn-app btn-ufop" href="{{ route('pdf2', array('id' => $plano->id)) }}" target="_blank" data-toggle="tooltip" id="visu" title="Expandir a tela com os dados do plano"><i class="fa fa-expand" aria-hidden="true"></i> Expandir</a>
            </div>
            <!-- <div class="btn-group" style="border:1px solid white">
                <a class="btn btn-app btn-ufop" href="{{ route('pdf', array('id' => $plano->id)) }}" target="_blank" data-toggle="tooltip" title="Gerar um pdf"><i class="fa fa-file-pdf-o"></i> PDF</a>
            </div> -->
        </div>
    </div>

    <!-- CORPO -->
    <div class='col-md-8 col-md-offset-2'>
        <div class="box box-primary-ufop">
            <div class="box-header">
                <div class="table-responsive">
                    <table id="ctitle">
                        <tr>
                            <td id="rep" width="20%">
                                <img src="{{URL::asset('/img/Republica-logo.jpeg')}}" alt="logo republica federativa do brasil">
                            </td>
                            <td id="text" width="60%" colspan="2">
                                <h4>UNIVERSIDADE FEDERAL DE OURO PRETO <br>PRÓ-REITORIA DE GRADUAÇÃO<br>PLANO DE ENSINO</h4>
                            </td>
                            <td id="ufop" width="20">
                                <img src="{{URL::asset('/img/Logomarca-min.jpg')}}" alt="logo ufop">
                                
                            </td>
                        </tr>            
                    </table>
                </div>
            </div><!-- end box-header -->
            
            <div class="box-body">
                <div class="table-responsive">
                    <table>
                        <tr>
                            <td colspan="3" width =75% >
                                <div class="pull-left">Nome do Componente Curricular em português:</div><br>
                                <strong>{{$plano->turma->disciplina->nome}}</strong>
                                <br><br>
                                <div class="pull-left">Nome do Componente Curricular em inglês:</div><br>
                                {{$plano->turma->disciplina->name}}
                            </td>
                            <td colspan="3" class="pull-top">
                                <div class="pull-left">
                                    Código: {{$plano->turma->disciplina->codigo}}
                                </div> 
                            </td>
                        </tr>

                        <tr>
                            <td  colspan="3" class="nob_border">
                                <div class="pull-left">Nome e sigla do departamento:</div><br>
                                <div class="pull-left">{{$plano->turma->disciplina->departamento->nome}} - {{$plano->turma->disciplina->departamento->sigla}}</div>
                            </td>
                            <td colspan="3" class="nob_border">
                                <div class="pull-left">Unidade acadêmica: </div><br>
                                <div class="pull-left">ICEA</div>
                            </td>
                        </tr>
                    </table>
                </div>
                <div class="table-responsive">
                    <table>
                        <tr>
                            <td colspan="5">
                                <div class="pull-left">Nome do docente:</div><br>
                                <div class="pull-left">
                                    @php ($cont=0)
                                    @foreach($plano->turma->users as $usuario)
                                        @if($cont>=1)
                                            - {{ $usuario->name}}
                                        @else
                                            {{ $usuario->name}}
                                        @endif
                                        @php ($cont++)
                                    @endforeach
                                </div>
                            </td>
                        </tr>

                        <tr>
                            <td colspan="2" class="nob_border">
                                Carga horária semestral<br>
                                {{$plano->turma->disciplina->chsemestral}} horas
                            </td>
                            <td colspan="2" class="nob_border">
                                Carga horária semanal teórica<br>
                                {{$plano->turma->disciplina->chsemanalt}} horas/aula
                            </td>
                            <td colspan="2" class="nob_border">
                                Carga horária semanal prática<br>
                                {{$plano->turma->disciplina->chsemanalp}} horas/aula
                            </td>
                        </tr>
                    </table>
                </div>
                <div class="table-responsive">
                    <table>
                        <tr>
                            <td colspan="5">
                                <div class="pull-left">Data de aprovação na assembléia departamental: {!!$plano->aprovacao!!}</div>
                            </td>
                        </tr>

                        <tr>
                            <td colspan="5">
                                <div class="pull-left">Ementa: </div><br>
                                {!!$plano->ementa!!}
                            </td>
                        </tr>

                        <tr>
                            <td colspan="5">
                                <div class="pull-left">Conteúdo programático: </div><br>
                                {!!$plano->conteudo!!}
                            </td>
                        </tr>

                        <tr>
                            <td colspan="5">
                                <div class="pull-left">Objetivos: </div><br>
                                {!!$plano->objetivo!!}
                            </td>
                        </tr>

                        <tr>
                            <td colspan="5">
                                <div class="pull-left">Metodologia: </div><br>
                                {!!$plano->metodologia!!}
                            </td>
                        </tr>
                    </table>
                </div>
                <div class="cronograma">
                    <div class="pull-left">
                        Cronograma:
                    </div><br>
                    <h4 class="margembZero">Critérios de Avaliação</h4>
                    <div class="table-responsive">
                        <table class="margemtZero margembZero" id="table">
                            <thead>
                                <tr>
                                    <th>Descrição da avaliação</th>
                                    <th>Peso da avaliação (%)</th>
                                    <th width="20%">Data</th>
                                    <th width="60%">Conteúdo avaliado</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($plano->exames as $e)
                                <tr>
                                    <td>{{$e->descricao}}</td>
                                    <td>{{$e->peso}}</td>
                                    <td>{{$e->data}}</td>
                                    <td>{{$e->conteudo}}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                    <div class="table-responsive">
                        <table class="margemtZero">
                            @if(count($plano->observacoes))
                                <tr>
                                    <td colspan="5">
                                        <div class="pull-left">{!!$plano->observacoes!!}</div>
                                    </td>
                                </tr>
                            @endif
                        </table>
                    </div><br>
                          
                    @if($plano->tipo == 1)

                    <h4>Planejamento das Aulas (sujeito a mudanças no decorrer do semestre)</h4>
                    <div class="table-responsive">
                        <table>
                            <thead>
                                <tr>
                                    <th>Aula</th>
                                    <th>Prática/<br>Teórica</th>
                                    <th width="20%">Data</th>
                                    <th width="60%">Conteúdo Previsto</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($plano->planejamentoAulas as $p)
                                <tr>
                                    <td>{{$p->aula}}</td>
                                    <td>{{$p->tipo}}</td>
                                    <td>{{$p->data}}</td>
                                    <td>{{$p->conteudo}}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
                        <p class="margemtZero pull-left"><strong><u>Atenção:</u></strong> No planejamento acima, cada "aula" corresponde a duas aulas de 50 minutos ou 1h 40 minutos.</p>
                    </div>
                                        
                    @else
                        <h4>Planejamento das Aulas (sujeito a mudanças no decorrer do semestre)</h4>
                        <div class="table-responsive">
                            <table>
                                <thead>
                                    <tr>
                                        <th>Unidade</th>
                                        <th>Hora/Aula</th>
                                        <th>Descrição</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($plano->planejamentoUnidades as $p)
                                    <tr>
                                        <td>{{$p->unidade}}</td>
                                        <td>{{$p->hora_aula}}</td>
                                        <td>{{$p->descricao}}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        </div>
                    @endif
                </div><br>
                <div class="table-responsive">
                    <table>
                        <tr>
                            <td colspan="5">
                                <div class="pull-left">Bibliografia Básica:</div><br><br>
                                <div class="pull-left">{!!$plano->bibliografiab!!}</div>
                            </td>
                        </tr>

                        <tr>
                            <td colspan="5">
                                <div class="pull-left">Bibliografia Complementar:</div><br><br>
                                <div class="pull-left">{!!$plano->bibliografiac!!}</div>
                            </td>
                        </tr>
                    </table>
                </div>
            </div>
        </div><!-- end box box-primary-ufop -->
    </div><!-- end col-sm-12 col-md-8 -->

    <!-- MODAL -->
    <div id="confirm_envio" class="modal fade modal_zindex">
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-body">
                    <h4>Foi verificado que o plano possui pendências não obrigatórias:</h4><br>
                    <ul>
                        <span id="msg-modal"></span>
                    </ul>
                    <h4>Deseja continuar com o envio para análise?</h4>
                    <div class="modal-footer">
                        <a href="/planos/{{$plano->id}}/verificacao" id="modal_envio" class="btn btn-danger">Continuar</a>
                        <button type="button" data-dismiss="modal" id="cancel" class="btn btn-default">Cancelar</button>
                    </div>
                </div>
            </div>
        </div>
    </div> 

</div><!-- end row -->    
@endsection
